# OpenAI / Codex Review Notes

Flato Design MCP uses a hosted MCP server with browser OAuth.

Review materials to prepare:

- MCP server URL: `https://api.flato.ai/api/mcp/editor`
- Authentication model: browser OAuth
- Exposed read tools and write tools
- Write-action behavior and user-visible project effects
- OAuth revoke behavior
- Privacy policy and terms links
- Credits and paid image generation/editing boundaries
- Logging, abuse control, and support contact
- Demo prompts and screenshots
